...
t="developer"
print(t[::-1][2:6][::-1]) # First make it as reverse then get the 2-6 index values next reverse that 2-6 index values

#write code for the i/p=aaaabbbcc as o/p=4a3b2c

x=input("Enter your characters : ") #aaaabbbccd
y=1
s=''
for i in range(1,len(x)):
    if x[i]==x[i-1]:          #a==a y=2 -->a==a y=3 -->a==a y=4 a!=b so go to else,b==b y=2 -->b==b y=3 b!=c go to else
        y+=1                   #like that checks the c as well as d
    else:
        s+=str(y)+x[i-1]    #"4"+a --> "4a"+"3"+b
        y=1                 # y=1
    if i==len(x)-1:
        s+=str(y)+x[i]  # it is for the last element if there is no match means it will display 1 with respective character
print(s)
